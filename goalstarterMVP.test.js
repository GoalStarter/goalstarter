...
describe('testing PUT with userid',function(){
  it('responds to POST /home/comment/:123',async(done)=>{
    const Body= {id: "321", author: "Steven Huang", comment: "abcdesef"}
      await request.post('/home/comment/:123').send(Body).expect(200)  
      done(); 
  })
})


describe('testing PUT with wrong userid',function(){
  it('responds to POST /home/comment/:321',async(done)=>{
    const Body= {id: "3211213131", author: "Not exist", comment: "ccdddd"}
      await request.post('/home/comment/:321').send(Body).expect(404);
      done();    
  })
})

describe('testing PUT with wrong userid',function(){
  it('responds to POST /home/create_goal/:userid',async(done)=>{
    const Body= {id: "123"}
      await request.post('/home/like/:123').send(Body).expect(200);
      done();   
  })
})
...
describe('testing PUT with wrong userid',function(){
  it('responds to POST /home/create_goal/:userid',async(done)=>{
    const Body= {id: "321"}
      await request.post('/home/like/:321').send(Body).expect(404);
      done();   
  })
})
